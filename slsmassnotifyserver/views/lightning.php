...
<div class="container-fluid sls-lightning-page">
	...
	<div class="sls-card">
		...
		<div class="sls-card-body">
			<p><?php echo _('Validate the applied client ID, protected client secret, and every enabled cloud-to-ground Lightning trigger area without sending an alert. Verification updates Dashboard health and refreshes the provider usage snapshot, but it also makes an additional query for each enabled area.'); ?></p>
	...
	<form method="post" action="config.php?display=slsmassnotifyserver_lightning">
		...
		<div class="sls-card">
			...
			<div class="sls-card-body">
				...
				<p class="sls-required-note"><?php echo _('Cloud-to-ground strikes are queried only. A cluster creates one alert per area when it first appears inside that radius and must clear before another alert. Standard Xweather lightning data covers the past 5 minutes, so 5 minutes is the longest gap-free query period. Selecting 6–10 minutes can miss strikes unless your subscription includes extended lightning history.'); ?></p>
				...
				<div class="alert alert-warning sls-adaptive-guidance"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> <?php echo _('Adaptive mode protects quota but can miss lightning that occurs without an NWS thunderstorm event. With multiple active trigger zones, multiple Xweather area queries may run in the same polling cycle. The quota governor may pause storm-mode polling if its protected token budget is depleted.'); ?></div>
			</div>
		</div>

		<div class="sls-card">
			<div class="sls-card-header sls-card-header-flex">
				<div class="sls-card-heading-copy"><h3><i class="fa fa-map-marker text-danger" aria-hidden="true"></i> <?php echo _('Lightning Trigger Areas'); ?> <span class="label label-success sls-lightning-labs-badge"><i class="fa fa-flask" aria-hidden="true"></i> <?php echo _('Labs'); ?></span></h3><small><?php echo _('Up to five independently routed areas, each with its own Weather.gov trigger, Xweather radius, and recipients.'); ?></small></div>
		...
		<div class="modal fade sls-lightning-modal" id="sls-lightning-group-manager" tabindex="-1" role="dialog" aria-labelledby="sls-lightning-group-manager-title" aria-hidden="true">
			<div class="modal-dialog"><div class="modal-content">
				...
				<div class="modal-body">
					<div class="alert alert-warning"><i class="fa fa-flask" aria-hidden="true"></i> <strong><?php echo _('Labs · token usage'); ?></strong> <?php echo _('You can configure up to five areas. Multiple areas under active storm triggers can query Xweather concurrently and consume the account allowance faster.'); ?></div>
		...
		<script type="text/template" id="sls-lightning-template">
			<div class="sls-lightning-editor" data-lightning-editor>
				...
				<div class="row"><div class="col-md-6"><div class="form-group"><label><?php echo _('Xweather Location'); ?></label><input class="form-control" data-lightning-field="location" maxlength="120" placeholder="30.5083,-97.6789 or Round Rock, TX"><p class="help-block"><?php echo _('Center point for this area’s Xweather query.'); ?></p></div></div><div class="col-md-2"><div class="form-group"><label><?php echo _('Radius'); ?></label><div class="input-group"><input class="form-control" data-lightning-field="radius_miles" type="number" min="1" max="62" value="25"><span class="input-group-addon"><?php echo _('mi'); ?></span></div></div></div><div class="col-md-4"><div class="form-group"><label><?php echo _('After this storm clears'); ?></label><select class="form-control" data-lightning-field="all_clear"><option value="none" selected><?php echo _('Do nothing'); ?></option><option value="send"><?php echo _('Send area all clear'); ?></option></select></div></div></div>
				<span class="sls-editor-section-label"><?php echo _('Area delivery routes'); ?></span>
...
(function(){
	...
	var testForm=document.getElementById('sls-lightning-test-form');var testButton=document.getElementById('sls-lightning-test-submit');var testResult=document.getElementById('sls-lightning-test-result');var selectionError=document.getElementById('sls-lightning-test-selection-error');var cooldown=document.getElementById('sls-lightning-test-cooldown');var remaining=cooldown?parseInt(cooldown.getAttribute('data-remaining')||'0',10)||0:0;var hasTestGroups=<?php echo $testGroups ? 'true' : 'false'; ?>;
	function selectedTests(){return testForm?testForm.querySelectorAll('input[name="lightning_group_ids[]"]:checked').length:0;}
	function renderCooldown(){if(!testButton||!cooldown)return;testButton.disabled=remaining>0||!hasTestGroups;if(remaining>0)cooldown.textContent=<?php echo json_encode(_('Available again in')); ?>+' '+remaining+' '+<?php echo json_encode(_('seconds')); ?>;else cooldown.textContent='';}
	renderCooldown();window.setInterval(function(){if(remaining>0){remaining-=1;renderCooldown();}},1000);
	if(testForm&&testButton&&testResult){testForm.addEventListener('submit',function(event){event.preventDefault();if(remaining>0)return;if(selectedTests()<1){if(selectionError)selectionError.style.display='block';return;}if(selectionError)selectionError.style.display='none';if(!window.confirm(<?php echo json_encode(_('Send a Lightning test to the selected applied trigger areas?')); ?>))return;testButton.disabled=true;testResult.style.display='block';testResult.className='alert alert-info';testResult.innerHTML='<i class="fa fa-spinner fa-spin" aria-hidden="true"></i> '+<?php echo json_encode(_('Running selected phone, audio, and desktop delivery checks…')); ?>;fetch(testForm.action,{method:'POST',credentials:'same-origin',body:new FormData(testForm)}).then(function(response){if(!response.ok)throw new Error('http_'+response.status);return response.json();}).then(function(data){var ok=!!(data&&data.success);var message=data&&data.message?String(data.message):<?php echo json_encode(_('Lightning test request finished.')); ?>;var errors=data&&Array.isArray(data.errors)?data.errors.filter(Boolean):[];testResult.className='alert alert-'+(ok?'success':'danger');testResult.textContent=message+(errors.length?' '+errors.join(' '):'');if(data&&data.cooldown)remaining=parseInt(data.cooldown.remaining||'0',10)||0;if(!ok)window.alert(<?php echo json_encode(_('Lightning test error')); ?>+'\n\n'+message+(errors.length?'\n\n'+errors.join('\n'):''));renderCooldown();}).catch(function(){testResult.className='alert alert-danger';testResult.textContent=<?php echo json_encode(_('Lightning test request failed before delivery could be confirmed.')); ?>;window.alert(<?php echo json_encode(_('Lightning test error')); ?>+'\n\n'+<?php echo json_encode(_('The PBX did not return a confirmed delivery result. Review Notification Logs.')); ?>);renderCooldown();});});}
}());
</script>
