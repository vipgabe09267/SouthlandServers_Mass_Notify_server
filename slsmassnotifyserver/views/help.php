...
<div class="container-fluid">
	...
	<ul>
		...
		<li><?php echo _('General Settings shows the installed package version and whether the known release status is LATEST or an update is available.'); ?></li>
			<li><?php echo _('Version 0.0.9 adds calendar scheduling, configurable regular-announcement display expiry, a portable alert-email sender domain, clearer Lightning test-call faults, and broader cross-PBX installer validation. The installer checks native prerequisites, every required Asterisk function/application and restart provider, the configured local AMI host/port, endpoint discovery, and effective PJSIP paging routes without modifying phone provisioning or SIP peers.'); ?></li>
	...
	<ul>
		...
		<li><?php echo _('Scheduling creates one-time announcements for one or more PBX-local calendar dates. Each schedule can select phones, groups, desktops, audio mode, Piper voice, volume, tones, and the Labs colored Yealink presentation. The worker checks once per minute and retries pre-delivery cooldown, busy, temporarily offline, or no-audio-target conditions inside a 15-minute grace window. It revalidates the live schedule before claiming it; a claimed delivery is never replayed after an interrupted worker. Failed or missed items must be re-armed with a new future date, while uncertain items require review.'); ?></li>
		<li><?php echo _('Scheduling uses the PBX operating-system timezone and rejects missing or ambiguous daylight-saving times. Dashboard health reports a mismatch with the FreePBX PHP timezone. Imported or restored schedules are disabled automatically because their PBX-local execution history is not included in a portable config.'); ?></li>
		...
		<li><?php echo _('Notification Destinations includes Email Sender Domain. Fresh installs use the detected PBX domain, the sender local part remains fixed as no-reply, and legacy configurations inherit the domain from a valid earlier sender address. The setting is stored in the protected central config and can be changed through the validated Control API configuration allowlist.'); ?></li>
		<li><?php echo _('Manual NWS tests use the same direct audio context and SIP NOTIFY sender as live alerts and can target all or selected weather-zone groups. A test succeeds only after Asterisk archives every targeted call as completed and the SIP NOTIFY sender accepts every requested contact. Tests and test-fault reports remain local and do not send email or Discord notifications.'); ?></li>
	...
	<ol>
		...
		<li><?php echo _('Choose whether to enable Weather Alerts. If enabled, configure up to five named U.S. weather.gov zone/county groups and select recipient extensions for each group.'); ?></li>
		<li><?php echo _('Choose whether to enable Labs Lightning Alerts. If enabled, configure Xweather credentials, one Weather Alert trigger zone, location, radius, recipients, and independent quiet hours.'); ?></li>
		<li><?php echo _('Review Control API access, desktop clients, paging audio, TTS voices and volume, notification destinations, Email Sender Domain, and log retention. The desktop-client table keeps approximately five rows visible, then scrolls with a sticky header.'); ?></li>
	</ol>
	...
	<ul>
		...
		<li><?php echo _('The canonical mail_from_domain value is included in this protected config and produces the fixed alert sender no-reply@&lt;domain&gt;. Older valid mail_from_addr values supply the domain when the canonical key is absent.'); ?></li>
	...
	<ul>
		...
		<li><code>POST {"action":"update_config","settings":{"mail_from_domain":"example.com"},"apply":false}</code> <?php echo _('stages a validated alert-email sender domain. The resulting sender is no-reply@example.com.'); ?></li>
	</ul>

	<h3><?php echo _('Email and Discord'); ?></h3>
	<p><?php echo _('Shared Weather and Lightning destinations are managed from the Notification Destinations popup in General Settings. Add or remove validated recipient email addresses there, set the Email Sender Domain, and optionally store one Discord webhook. The sender is always no-reply@ followed by the validated domain; fresh installs use the detected PBX hostname.'); ?></p>
	<p><?php echo _('Changing Email Sender Domain affects only the identity used by Mass Notify alert messages. It does not configure Postfix, an SMTP relay, DNS, SPF, DKIM, DMARC, or PTR/reverse DNS. Configure those separately so the selected domain is authorized to send from this PBX.'); ?></p>
	<p><?php echo _('Live alert email is sent through the local sendmail path as an always-branded Southland Servers HTML card with an embedded logo and a plain-text alternative. Live Discord delivery uses a compact branded embed with the SLS identity, public logo image/avatar, event-aware emoji/color, concise delivery fields, a normalized ISO timestamp, and an urgency footer. Manual Weather and Lightning tests do not send either destination. The public logo URL is built from the automatically detected PBX hostname. Destinations, sender domain, and webhook are stored in the protected central config.'); ?></p>
