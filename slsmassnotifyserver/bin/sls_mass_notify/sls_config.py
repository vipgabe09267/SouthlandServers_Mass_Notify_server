...
def main():
    ...
    values = {
        ...
        "NWS_ZONE": zone,
        "SLS_OPENING_TONE": re.sub(r"[^A-Za-z0-9_-]", "", text(data.get("opening_tone"))),
        "SLS_CLOSING_TONE": re.sub(r"[^A-Za-z0-9_-]", "", text(data.get("closing_tone"))),
        "PIPER_BIN": "/usr/local/bin/sls_mass_notify/piper/venv/bin/piper",
        "PIPER_NWS_VOICE": validated_voice(data.get("nws_piper_voice") or data.get("piper_voice")),
        "PIPER_ANNOUNCEMENT_VOICE": validated_voice(data.get("announcement_piper_voice")),
        "PIPER_NWS_VOLUME": scalar(data.get("nws_tts_volume"), 85),
        "PIPER_ANNOUNCEMENT_VOLUME": scalar(data.get("announcement_tts_volume"), 50),
